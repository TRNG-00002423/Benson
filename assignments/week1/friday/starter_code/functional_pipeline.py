...
by_module_duration = sorted(test_results, key=lambda test: (test["name"], test["duration_ms"]))
...
failures = filter(lambda test: test["status"] == "fail", test_results)

# TODO 3b: Get all tests slower than 1500ms.
slow_tests = filter(lambda test: test["duration_ms"] < 1500, test_results)
...
critical = filter(lambda test: test["duration_ms"] < 1500 and test["status"] == "fail", test_results)
...
# Expected: 15560
total_duration = reduce(lambda total, test: total + test["duration_ms"], test_results, 0)

# TODO 4b: Total duration of FAILED tests only.
# Hint: chain filter() before reduce().
total_fail_duration = reduce(lambda total, test: total + test["duration_ms"], 
                             filter(lambda test: test["status"] == "fail", test_results), 0)

# TODO 4c: Find the name of the test with the LONGEST name (by character count).
longest_name = reduce(lambda longest, test: test["name"] if len(test["name"]) > len(longest) else longest, test_results, "")

# TODO 4d: Build a module summary dict counting tests per module.
# Expected: {'auth': 3, 'search': 3, 'checkout': 3, 'profile': 3}
module_counts = reduce(lambda count, test: {**count, test["module"] : count.get(test["module"], 0) + 1}, test_results, {})
...
name_to_duration = {name: duration for name, duration in zip(names, durations)}
...
avg_fail_duration = reduce(lambda total, curr_ms: total + curr_ms, map(lambda test: test["duration_ms"], 
                        filter(lambda test: test["status"] == "fail", test_results)), 0) / len(list(filter(lambda test: test["status"] == "fail", test_results)))
